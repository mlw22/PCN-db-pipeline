...
def fetch_gbk(RefSeq_list_file, assembly_list_file, ftp_path_file):  
    list_of_ftp_paths = []
    with open(RefSeq_list_file, "r") as refseq_list:
        gcf_ids = refseq_list.read().splitlines()
    with open(assembly_list_file, "r") as assembly_list:
        asm_ids = assembly_list.read().splitlines()

    num_paths = 0  # Variable to track the number of paths collected
    for refseq_accession, assembly_accession in zip(gcf_ids, asm_ids):
    ...
    if list_of_ftp_paths:  # Check if any paths were retrieved
        print(f"List of paths: {list_of_ftp_paths}")
    else:
    ...
    try:
        ...
        print(f"Error writing to file: {ftp_path_file}")
    else:
        print(f"Paths written to: {ftp_path_file}")
...
refgenomes_dir = "../results/PCN_pipeline/ref-genomes/"
...
fetch_gbk(RefSeq_list_file, assembly_list_file, ftp_path_file)

create_genome_metadatacsv(ftp_path_file, table_outfile, genome_metadatacsv)
Hawkey2022_genomeID_to_SRA_ID_dict = make_genome_to_SRA_dict(genome_metadatacsv)
...
def pipeline_main():
    ...
    create_genome_metadatacsv(ftp_path_file, table_outfile, genome_metadatacsv)
    Hawkey2022_genomeID_to_SRA_ID_dict = make_genome_to_SRA_dict(genome_metadatacsv)
    download_fastq_reads(SRA_data_dir, table_outfile)
    make_Hawkey_fasta_refs_for_kallisto(refgenomes_dir, kallisto_ref_dir)
    make_Hawkey_kallisto_indices(kallisto_ref_dir, kallisto_index_dir)
    run_kallisto_quant(Hawkey2022_genomeID_to_SRA_ID_dict, kallisto_index_dir, SRA_data_dir, kallisto_quant_results_dir)
    measure_Hawkey2022_replicon_copy_numbers(kallisto_quant_results_dir, copy_number_csv_file)
